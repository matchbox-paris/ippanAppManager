...
const { NODE_ENV, ROOT, TMP, DEBUG, VERBOSE } = function(){
    return {
      ...
      , TMP: process.env.TMP || CONFIG.TMP || CONFIG.TMP_PATH || CONFIG.PATH_TMP
        ...
        constructor: function( { debug=false, verbose=false, transpile = true
            ...
            bundlers.get(this).set("ready", Promise.resolve()
                ...
                .then(() => new Promise(resolve => {
                    ...
                    if ( this.minify &&
                         [...this.babel_presets]
                            .filter(v => v == "minify" || (v[0] && v[0] == "minify")).length > 0
                       )
                      this.babel_presets.add(["minify", this.minify_opts])
      ...
      , bundle: { enumerable: true,
            value: function(cb){
                ...
                .then(() => new Promise((resolve, reject) =>{
                    ...
                    config.resolve = {
                        modules: [ ...bundlers.get(this).get("resolve_modules"), resolvePath(this.context, "./node_modules") ].filter(v => !!v)
                      , alias: {}
                    }
                    ...
                    console.log("xxxx", dirname(this.to_js), dirname(this.to_css), joinPath(relativePath(dirname(this.to_js), dirname(this.to_css)), basename(this.to_css)))
                    config.plugins = [
