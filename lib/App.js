...
const { Server } = require("ippanserver/lib/Server")
const { WebSocketUpgrade } = require("ippanserver/lib/WebSocketUpgrade")
...
module.exports.App = klass(Node, statics => {
    ...
    return {
        constructor: function({ watch:allow_watch=true, websocket:{port:ws_port} ={} } ={} ){
            Node.call(this)
            ...
            .then(() => new Promise(resolve => {
                ...
                process.env.VERBOSE = argv.has("--VERBOSE") || argv.has("--verbose") || argv.has("-v") || process.env.VERBOSE || 0
                process.env.DEBUG = argv.has("--DEBUG") || argv.has("--debug") || argv.has("-d") || process.env.DEBUG || 0
            ...
            .then(() => new Promise(resolve => {
                ...
                this.open_websocket(ws_port, resolve)
            }))
            .then(() => this.dispatchEvent("beforecp"))
      ...
      , websocketServer: { enumerable: true,
            get: function(){ return apps.get(this).get("ws_server") }
        }
      , open_websocket: { enumerable: true,
            value: function(port, cb){
                cb = typeOf(cb) == "function" ? cb : Function.prototype

                const server = new Server()
                const wsupgrade = this.appendChild( new WebSocketUpgrade(server) )

                apps.get(this).set("ws_server", { server, wsupgrade })
